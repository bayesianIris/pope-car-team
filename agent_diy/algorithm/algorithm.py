...
Enhanced PPO algorithm implementation for DIY.
DIY的增强PPO算法实现。
...
class Algorithm:
    def __init__(self, model, optimizer, device=None, logger=None, monitor=None):
        ...
        self.action_num = Config.ACTION_NUM  # 16
        self.value_num = Config.VALUE_NUM    # 1
    ...
    def learn(self, list_sample_data):
        ...
        # Stack all sample data into tensors
    ...
    def _compute_loss(
        ...
        one_hot = torch.nn.functional.one_hot(old_action[:, 0].long(), self.action_num).float()
    ...
    def _masked_softmax(self, logits, legal_action):
        """Apply masked softmax to enforce legal actions only.
        
        应用掩码softmax以仅允许合法动作。
        """
        # Convert legal_action to mask (-inf for illegal, 0 for legal)
        legal_mask = legal_action.float()
        illegal_mask = (1.0 - legal_mask) * -1e9
        masked_logits = logits + illegal_mask

        # Softmax
        prob_dist = torch.softmax(masked_logits, dim=1)

        # Ensure zero probability for illegal actions (numerical stability)
        prob_dist = prob_dist * legal_mask

        # Renormalize
        prob_dist = prob_dist / (prob_dist.sum(dim=1, keepdim=True) + 1e-9)
